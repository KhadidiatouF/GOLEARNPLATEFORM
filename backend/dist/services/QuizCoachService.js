...
exports.QuizCoachService = void 0;
const env_1 = require("../config/env");
class QuizCoachService {
    static COACH_NAME = "Professeur Awa";
    async generateFeedback(payload) {
        if (env_1.config.OPENAI_API_KEY && env_1.config.OPENAI_MODEL) {
            try {
    ...
    async generateWithOpenAI(payload) {
        const wrongQuestions = payload.questions.filter((question) => question.selectedIndex !== question.correctIndex);
        const prompt = [
            ...
            JSON.stringify(wrongQuestions.map((question) => ({
    ...
    generateFallbackFeedback(payload) {
        const wrongQuestions = payload.questions.filter((question) => question.selectedIndex !== question.correctIndex);
...
exports.QuizCoachService = QuizCoachService;
//# sourceMappingURL=QuizCoachService.js.map