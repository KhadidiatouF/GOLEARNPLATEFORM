...
class MailService {
    static async sendProfessorCredentialsEmail({ recipientEmail, recipientName, login, temporaryPassword, resetLink, }) {
        if (!mailer_1.mailerConfig.enabled) {
            console.warn(`[MAIL DISABLED] Email professeur non envoye a ${recipientEmail}. Configure EMAIL_USER et EMAIL_PASS pour activer l'envoi.`);
            return;
        }
    ...
    `;
        await mailer_1.transporter.sendMail({
            from: mailer_1.mailerConfig.from,
            to: recipientEmail,
            subject,
            text,
            html,
